...
export function loadConfig(repoRoot: string): ScaffoldConfig {
  ...
  if (!raw.azure_openai_endpoint) {
    console.error(`Error: azure_openai_endpoint is required in .scaffold-config.yml`);
    process.exit(1);
  }

  if (!raw.azure_openai_deployment) {
    console.error(`Error: azure_openai_deployment is required in .scaffold-config.yml`);
    process.exit(1);
  }
